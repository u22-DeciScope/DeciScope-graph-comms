...
namespace EchoBot.Services
{
    public sealed class BotMeetingStatusReporter : IBotMeetingStatusReporter
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var metadataUrl = BuildMetadataUrl(options.ApiUrl, sessionId);
            ...
            for (var attempt = 1; attempt <= MaxAttempts; attempt++)
            {
                try
                {
                    using var request = new HttpRequestMessage(HttpMethod.Patch, metadataUrl);
                    request.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    request.Headers.Add("X-DeciScope-Api-Key", options.ApiKey);
                    request.Content = new StringContent(json, Encoding.UTF8, "application/json");
                    ...
                    {
                        logger.LogWarning(
                            "Meeting metadata report failed. SessionId={SessionId}; Title={Title}; TitleSource={TitleSource}; Provider={Provider}; ThreadId={ThreadId}; ExternalMeetingId={ExternalMeetingId}; StatusCode={StatusCode}; Attempt={Attempt}; MaxAttempts={MaxAttempts}",
                            sessionId,
                            metadata.Title,
                    ...
                    logger.LogInformation(
                        "Meeting metadata report succeeded. SessionId={SessionId}; Title={Title}; TitleSource={TitleSource}; Provider={Provider}; ThreadId={ThreadId}; ExternalMeetingId={ExternalMeetingId}; StatusCode={StatusCode}; Attempt={Attempt}",
                        sessionId,
                        metadata.Title,
                        metadata.TitleSource,
                        metadata.Provider,
                ...
                catch (Exception ex) when (ex is HttpRequestException || ex is TaskCanceledException || ex is OperationCanceledException)
                {
                    logger.LogWarning(
                        ex,
                        "Meeting metadata report retry. SessionId={SessionId}; Title={Title}; TitleSource={TitleSource}; Provider={Provider}; ThreadId={ThreadId}; ExternalMeetingId={ExternalMeetingId}; Attempt={Attempt}; MaxAttempts={MaxAttempts}; Error={Error}",
                        sessionId,
                        metadata.Title,
                        metadata.TitleSource,
                        metadata.Provider,
                    ...
                    if (attempt >= MaxAttempts)
                    {
                        logger.LogError(
                            ex,
                            "Meeting metadata report failed. SessionId={SessionId}; Title={Title}; TitleSource={TitleSource}; Provider={Provider}; ThreadId={ThreadId}; ExternalMeetingId={ExternalMeetingId}; Attempts={Attempts}; Error={Error}",
                            sessionId,
                            metadata.Title,
                            metadata.TitleSource,
                            metadata.Provider,
        ...
        public static Uri BuildMetadataUrl(Uri transcriptApiUrl, string sessionId)
        {
            var escapedSessionId = Uri.EscapeDataString(sessionId);
            var builder = new UriBuilder(transcriptApiUrl);
            var path = builder.Path;
